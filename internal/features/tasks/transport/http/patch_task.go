...
func (r *PatchTaskRequest) Validate() error {
	if r.Title.Set {
		// проверяем, что http-клиент не пытается удалить заголовок
		if r.Title.Value == nil {
			return fmt.Errorf("`Title` can't be NULL")
		}
		// если http-клиент передал заголовок, то количество символов нахоядтся в рамках допустимых значений
	...
	if r.Description.Set {
		if r.Description.Value != nil {
			...
			if descriptionLen < 1 || descriptionLen > 1000 {
				return fmt.Errorf("`Description` must be berween 1 and 1000 symbols")
			}
		}
	}
