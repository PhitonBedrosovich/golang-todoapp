...
// @Param         from query string false                           "Начало промежутка рассмотрения статистики (влючительно), формат: YYYY-MM-DD"
...
func toDTOFromDomain(statistics domain.Statistics) GetStatisticsResponse {
	...
	if statistics.TasksAverageCompletionTime != nil {
		// сначала результат выполнения положить в переменную, а потом взять указатель на эту переменную
